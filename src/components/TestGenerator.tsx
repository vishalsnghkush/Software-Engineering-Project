...
const DEFAULT_API_KEY = "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";

export default function TestGenerator({ autoGenerate, onBack }: TestGeneratorProps = {}) {
  // ─── Persisted API Key ───
  ...
  const [difficulty, setDifficulty] = useState(autoGenerate?.difficulty || "actual experience as in actual paper");
  const [numQuestions, setNumQuestions] = useState<number>(autoGenerate?.numQuestions || 5);

  useEffect(() => {
    if (generationType === "Subject Wise") {
      if (examName === "JEE" && !["Physics", "Chemistry", "Mathematics"].includes(subject)) setSubject("Physics");
      else if (examName === "NEET" && !["Physics", "Chemistry", "Biology"].includes(subject)) setSubject("Physics");
    }
    ...
    try {
      ...
      if (!res.ok) {
        if (generationType === "Complete Paper" && res.status === 500) {
          throw new Error("Free tier limit exceeded for generating a complete paper. Try a smaller 'Topic Wise' test or use a paid model.");
        }
  ...
  const startTest = async () => {
    ...
    let totalSeconds = 0;
    if (examName === "JEE") totalSeconds = Math.floor(qCount * ((180 * 60) / 75));
    else if (examName === "NEET") totalSeconds = Math.floor(qCount * ((200 * 60) / 200));
    else if (examName === "CUET") totalSeconds = Math.floor(qCount * ((60 * 60) / 50));
    else if (examName === "GATE") totalSeconds = Math.floor(qCount * ((180 * 60) / 65));
    ...
    if (autoGenerate && !autoTriggered.current) {
      ...
      fetch("/api/generate-test", {
        ...
        body: JSON.stringify({
          ...
          difficulty: autoGenerate.difficulty || "actual experience as in actual paper",
  ...
  useEffect(() => {
    ...
    const onVis = () => { if (document.visibilityState === "hidden") submitTest(); };
    const onFs = () => { if (!document.fullscreenElement) submitTest(); };
  ...
  const handleClear = () => {
    setUserAnswers(prev => { const copy = { ...prev }; delete copy[currentQ]; return copy; });
  };
  ...
  const submitTest = useCallback(() => {
    let marks = 0, correct = 0, incorrect = 0, unattempted = 0;
    let correctMark = 4, incorrectMark = 1;
    if (examName === "CUET") { correctMark = 5; incorrectMark = 1; }
    else if (examName === "GATE") { correctMark = 1; incorrectMark = 0.33; }

    questions.forEach((q, idx) => {
      const uA = userAnswers[idx], cA = q.correctAnswer || "";
      if (!uA) unattempted++;
      else if (uA === cA) { correct++; marks += correctMark; }
      else { incorrect++; marks -= incorrectMark; }
    });
    if (examName === "GATE") marks = parseFloat(marks.toFixed(2));

    setScoreData({ marks, correct, incorrect, unattempted, correctMark, incorrectMark, maxMarks: questions.length * correctMark });
    setAppState("results");
  ...
  const resetGenerator = () => {
    setAppState("setup"); setQuestions([]); setUserAnswers({}); setTimeLeft(null);
    setMarkedForReview(new Set()); setVisitedQuestions(new Set()); setCurrentQ(0);
  };
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
        {appState === "generating" ? (
          <>
            ...
            <h2 className="text-2xl font-extrabold text-white mb-2">Generating your test...</h2>
            <p className="text-slate-400 text-sm">
              Creating {autoGenerate.numQuestions} questions on <span className="text-indigo-400 font-bold">{autoGenerate.subject}</span> for {autoGenerate.examName}
            </p>
          </>
        ) : error ? (
          <>
            ...
            {onBack && (
              <button onClick={onBack} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-500 transition-colors">
                ← Back to Dashboard
              </button>
            )}
          </>
          ...
          <>
            <div className="w-16 h-16 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-6"></div>
            <h2 className="text-2xl font-extrabold text-white mb-2">Preparing...</h2>
          </>
        )}
      </div>
    );
  ...
  if (appState === "setup" || appState === "generating") {
    return (
      <div className="max-w-5xl mx-auto my-12 p-8 sm:p-12 bg-white/80 dark:bg-slate-900/70 backdrop-blur-3xl rounded-[3rem] shadow-2xl shadow-indigo-500/10 border border-white/50 dark:border-slate-700/50 transition-all min-h-[500px] relative overflow-hidden">
        ...
        <form onSubmit={handleGenerate} className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
          {/* Target Exam */}
          ...
          <div className="space-y-2">
            ...
            <select value={generationType} onChange={(e) => setGenerationType(e.target.value)} className="w-full px-5 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
              ...
              <option value="Complete Paper">🎯 Full Mock Test (All Subjects)</option>
            </select>
          </div>
          ...
          <div className="space-y-2 flex gap-4">
            <div className={`${generationType === "Complete Paper" ? "w-full" : "flex-1"}`}>
              ...
              <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="w-full px-5 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                ...
                <option value="actual experience as in actual paper">Actual Difficulty</option>
          ...
          {generationType === "Complete Paper" && (
            <div className="md:col-span-2 p-6 rounded-2xl bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border border-indigo-200 dark:border-indigo-800">
              <p className="text-base font-bold text-indigo-700 dark:text-indigo-300 mb-3">🎯 Full Mock Test — {examName}</p>
              {examName === "JEE" && (
                <div className="space-y-2 text-sm text-indigo-600 dark:text-indigo-400">
                  <p className="font-semibold">75 Questions • 3 Hours • Physics + Chemistry + Mathematics</p>
                  <div className="grid grid-cols-3 gap-3 mt-3">
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-indigo-100 dark:border-indigo-900">
                      <p className="font-black text-lg text-indigo-700 dark:text-indigo-300">25</p>
                  ...
                  <p className="text-xs mt-2 text-amber-600 dark:text-amber-400">⚠ Marking: +4 correct, −1 incorrect. Free AI models may struggle with 75 questions.</p>
                </div>
              )}
              {examName === "NEET" && (
                <div className="space-y-2 text-sm text-indigo-600 dark:text-indigo-400">
                  <p className="font-semibold">200 Questions • 3 hrs 20 min • Physics + Chemistry + Biology</p>
                  <div className="grid grid-cols-3 gap-3 mt-3">
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-indigo-100 dark:border-indigo-900">
                      <p className="font-black text-lg text-indigo-700 dark:text-indigo-300">50</p>
                      <p className="text-xs text-slate-500">Physics</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-indigo-100 dark:border-indigo-900">
                      <p className="font-black text-lg text-indigo-700 dark:text-indigo-300">50</p>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-indigo-100 dark:border-indigo-900">
                      <p className="font-black text-lg text-indigo-700 dark:text-indigo-300">100</p>
                      <p className="text-xs text-slate-500">Biology</p>
                    </div>
                  </div>
                  <p className="text-xs mt-2 text-amber-600 dark:text-amber-400">⚠ Marking: +4 correct, −1 incorrect. Free AI models may struggle with 200 questions.</p>
                </div>
              )}
              {examName === "CUET" && (
                <div className="space-y-2 text-sm text-indigo-600 dark:text-indigo-400">
                  <p className="font-semibold">50 Questions • 1 Hour • Complete Syllabus</p>
                  <p className="text-xs mt-2 text-amber-600 dark:text-amber-400">⚠ Marking: +5 correct, −1 incorrect.</p>
                </div>
              )}
              {examName === "GATE" && (
                <div className="space-y-2 text-sm text-indigo-600 dark:text-indigo-400">
                  <p className="font-semibold">65 Questions • 3 Hours • Complete Syllabus</p>
                  <p className="text-xs mt-2 text-amber-600 dark:text-amber-400">⚠ Marking: +1 correct, −0.33 incorrect.</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="md:col-span-2 mt-8">
            {generationType === "Complete Paper" ? (
              <div>
                <button type="button" disabled className="w-full py-5 px-8 rounded-2xl bg-slate-400 text-white text-xl font-black shadow-xl cursor-not-allowed opacity-70">
                ...
                <div className="mt-4 p-5 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700">
                  <p className="text-sm font-bold text-amber-700 dark:text-amber-300 mb-1">⚠️ No Free Model Available</p>
                  <p className="text-sm text-amber-600 dark:text-amber-400">
                    As of now, we have not found any free AI model capable of generating this many questions in a single request. 
                    The output gets truncated midway, resulting in broken JSON.
                  </p>
                  <p className="text-sm text-amber-600 dark:text-amber-400 mt-2 font-semibold">
                    💡 Please use <strong>&quot;Topic Wise&quot;</strong> or <strong>&quot;Subject Wise&quot;</strong> mode to generate smaller tests (up to 50 questions) for now.
                  </p>
                </div>
              </div>
            ) : (
    ...
    return (
      <div className="max-w-3xl mx-auto my-20">
        <div className="flex flex-col items-center justify-center py-10 px-6 bg-slate-50 dark:bg-slate-800/40 rounded-3xl border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-2 text-center">{examName} — {generationType}</p>
          <p className="text-slate-400 dark:text-slate-500 mb-8 max-w-sm text-center text-sm">
            The test will open in fullscreen CBT mode. Switching tabs or exiting fullscreen will auto-submit.
          </p>
  ...
  if (appState === "testing") {
    ...
    return (
      <section className="h-screen flex flex-col bg-white dark:bg-slate-900">
        {/* ── Header with Timer ── */}
        <Navbar isBordered className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
          <NavbarContent justify="start">
            <NavbarItem>
              <h1 className="font-bold text-lg text-slate-800 dark:text-white">{examName} — Question {currentQ + 1} of {questions.length}</h1>
            </NavbarItem>
          </NavbarContent>
        ...
        <section className="flex-1 h-full flex flex-col-reverse md:flex-row overflow-y-auto">
          ...
          <div className="overflow-auto h-full w-full p-6 md:p-10 flex flex-col space-y-6 bg-white dark:bg-slate-900">
            <h2 className="text-xl font-bold text-slate-800 dark:text-white">Question {currentQ + 1}</h2>
            <p className="text-lg text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{q?.question}</p>

            <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-4">Select your answer:</h3>

            <RadioGroup value={userAnswers[currentQ] || ""} onValueChange={handleSelectOption} className="mt-2 space-y-3">
              ...
              <Popover placement="bottom-end">
                ...
                <PopoverContent>
                  <div className="p-2 space-y-2">
                    <div className="flex gap-2 items-center"><QuestionButton label={1} questionStatus={QuestionStatus.NotVisited} /><p className="text-sm">Not Visited</p></div>
                    <div className="flex gap-2 items-center"><QuestionButton label={2} questionStatus={QuestionStatus.NotAnswered} /><p className="text-sm">Visited (No Answer)</p></div>
                    <div className="flex gap-2 items-center"><QuestionButton label={3} questionStatus={QuestionStatus.Answered} /><p className="text-sm">Answered</p></div>
                    <div className="flex gap-2 items-center"><QuestionButton label={4} questionStatus={QuestionStatus.Marked} /><p className="text-sm">Marked for Review</p></div>
                    <div className="flex gap-2 items-center"><QuestionButton label={5} questionStatus={QuestionStatus.AnsweredMarked} /><p className="text-sm">Answered &amp; Marked</p></div>
                  </div>
                </PopoverContent>
              </Popover>
              <div className="hidden md:flex flex-col gap-2 mt-4">
                <div className="flex gap-2 items-center"><QuestionButton label={1} questionStatus={QuestionStatus.NotVisited} /><p className="text-sm">Not Visited</p></div>
                <div className="flex gap-2 items-center"><QuestionButton label={2} questionStatus={QuestionStatus.NotAnswered} /><p className="text-sm">Visited</p></div>
                <div className="flex gap-2 items-center"><QuestionButton label={3} questionStatus={QuestionStatus.Answered} /><p className="text-sm">Answered</p></div>
                <div className="flex gap-2 items-center"><QuestionButton label={4} questionStatus={QuestionStatus.Marked} /><p className="text-sm">Marked</p></div>
                <div className="flex gap-2 items-center"><QuestionButton label={5} questionStatus={QuestionStatus.AnsweredMarked} /><p className="text-sm">Answered &amp; Marked</p></div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800 flex flex-col md:flex-row gap-3 p-4 pb-6 w-full border-t-2 border-slate-200 dark:border-slate-700 shadow-[0_-4px_20px_rgba(0,0,0,0.2)]">
          <div className="flex gap-3 w-full">
            <Button className="flex-1 md:flex-initial md:w-32 h-11 text-base font-bold" color="danger" variant="flat" onPress={handleClear}>
              Clear
            </Button>
            <Button className="flex-1 md:flex-initial md:w-40 h-11 text-base font-bold" color="secondary" variant="flat" onPress={handleMarkAndNext}>
              Mark &amp; Next
            </Button>
            <Button className="flex-1 md:flex-initial md:w-32 md:ml-auto h-11 text-base font-bold" color="success" variant="flat" onPress={handleSaveAndNext}>
              Save &amp; Next
            </Button>
          </div>
          <div className="flex gap-3">
            <Button className="flex-1 h-11 text-base font-bold" color="primary" variant="solid" onPress={submitTest}>
              Submit Test
            </Button>
          </div>
        </div>
    ...
    return (
      <div className="max-w-5xl mx-auto my-12 p-8 space-y-16">
        <div className="bg-gradient-to-b from-indigo-50 via-white to-purple-50 dark:from-slate-800 dark:via-slate-900 dark:to-indigo-950/30 p-12 sm:p-16 rounded-[3.5rem] text-center border border-white/60 dark:border-slate-700/50 shadow-2xl shadow-indigo-500/10 relative overflow-hidden backdrop-blur-2xl">
          <h3 className="text-2xl text-slate-500 dark:text-slate-400 font-bold mb-4 uppercase tracking-widest">Test Completed</h3>
          <div className="text-8xl font-black text-slate-800 dark:text-white mb-6">
            ...
            <span className="text-4xl text-slate-400 font-bold ml-2">/ {scoreData.maxMarks}</span>
          </div>
          <p className="text-xl font-medium text-slate-600 dark:text-slate-400 mb-6">Total Marks Scored</p>
          <div className="flex justify-center gap-6 mb-10 text-lg flex-wrap">
            <div className="bg-emerald-100 text-emerald-800 px-4 py-2 rounded-xl font-bold">Correct: {scoreData.correct} (+{parseFloat((scoreData.correct * scoreData.correctMark).toFixed(2))})</div>
            <div className="bg-rose-100 text-rose-800 px-4 py-2 rounded-xl font-bold">Incorrect: {scoreData.incorrect} (-{parseFloat((scoreData.incorrect * scoreData.incorrectMark).toFixed(2))})</div>
            <div className="bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold">Unattempted: {scoreData.unattempted} (0)</div>
          </div>
          <button onClick={resetGenerator} className="py-4 px-10 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold transition-all shadow-xl shadow-indigo-600/20 active:scale-[0.98]">
            Generate A New Test
          </button>
        </div>
        ...
        <div className="space-y-8">
          ...
          {questions.map((q, idx) => {
            ...
            return (
              <div key={idx} className={`p-8 rounded-3xl border-2 shadow-sm transition-all ${isCorrect ? 'border-emerald-200 bg-emerald-50/40' : 'border-rose-200 bg-rose-50/40'}`}>
                <p className="text-xl font-semibold text-slate-900 dark:text-white mb-6 leading-relaxed flex items-start">
                  <span className={`mr-4 border rounded-full px-4 py-1 text-sm font-black flex-shrink-0 mt-1 shadow-sm ${isCorrect ? 'text-emerald-700 bg-emerald-100 border-emerald-300' : 'text-rose-700 bg-rose-100 border-rose-300'}`}>Q{idx + 1}</span>
                  {q.question}
                </p>
                <div className="grid grid-cols-1 gap-3 ml-2 sm:ml-12 mb-6">
                  {q.options?.map((opt: string, optIdx: number) => {
                    ...
                    if (isActualCorrect) optStyle = 'border-emerald-500 bg-emerald-50 text-emerald-800 font-bold shadow-sm';
                    else if (isSelected && !isCorrect) optStyle = 'border-rose-400 bg-rose-50 text-rose-800 opacity-90 line-through decoration-rose-400/50';
                    return (
                ...
                {!isCorrect && (
                  <div className="ml-2 sm:ml-12 mt-6 p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4 sm:gap-12">
                    <div>
                      ...
                      <p className="text-base font-bold text-rose-600">{uAns || 'Skipped'}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-base font-bold text-emerald-600">{cAns}</p>
                    </div>
                  </div>
                )}
              </div>
