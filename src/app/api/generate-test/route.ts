...
Remember: plain text only, no LaTeX. Output raw JSON array only.`
    ...
    try {
      ...
      questions = questions.map(q => {
       ...
       try {
         const questionRegex = /\{"question"\s*:\s*"([^"]*?)"\s*,\s*"options"\s*:\s*\[([^\]]*?)\]\s*,\s*"correctAnswer"\s*:\s*"([^"]*?)"\}/g;
         let match;
         while ((match = questionRegex.exec(content)) !== null) {
           const opts = match[2].split(',').map(s => s.trim().replace(/^"|"$/g, ''));
           questions.push({
             question: match[1],
             options: opts.length >= 4 ? opts.slice(0, 4) : [...opts, ...Array(4 - opts.length).fill("None of the above")],
